...
description: "Allocate pixel-buffer image elements, draw shapes and sprites onto them, and flush changes to the screen with GUpdateImage."
---
...
| `>` | Skip one pixel to the right (preceded by optional count) |
...
A count prefix applies to the immediately following character or group. `>N` is shorthand for `N>` — it advances `N` pixels right.
...
Forces every image element to be fully retransferred to its `XImage` on the next render pass, bypassing the dirty-pixel cache. This is useful after a palette modification, since changing a color index affects how every existing pixel with that index appears but does not change the pixel buffer values themselves.
