---
title: "Window & Loop API Reference — LWXGL"
sidebarTitle: "Window"
description: "Complete reference for LWXGL window creation, the render loop, lifecycle functions, modals, and the palette management API."
---
...
Runs a fixed-FPS event loop that handles events, renders, and calls your per-frame callback on each tick. The loop uses `std::chrono::steady_clock` and will sleep or yield the thread between frames to stay close to the requested rate. It also measures per-frame work time and feeds it into the debug overlay's rolling average. The function returns only when `GWindowShouldClose()` becomes true.
...
<ParamField path="on_every" type="void (*)(int)" >
  Callback invoked once per rendered frame. Receives the current **tick counter** (starts at `0`, increments by `1` each frame). Pass `NULL` if no per-frame callback is needed.
</ParamField>
...
<ParamField path="r" type="unsigned char*" required>
  Pointer to a `unsigned char` that will receive the red component (0–255).
</ParamField>

<ParamField path="g" type="unsigned char*" required>
  Pointer to a `unsigned char` that will receive the green component (0–255).
</ParamField>

<ParamField path="b" type="unsigned char*" required>
  Pointer to a `unsigned char` that will receive the blue component (0–255).
</ParamField>
...
static void on_frame(int tick) {
    /* Called once per frame. tick starts at 0 and increments each frame. */
    (void)tick;
}
...
int main(void) {
    ...
    /* Run at 60 fps; on_frame is called every tick */
